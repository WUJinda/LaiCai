# -*- coding: utf-8 -*-
"""ATR动态阈值策略 - 融合三次回测结果（H2/H4/D1），生成统一报告 + 过程图"""
import json
import os
import sys
...
from _batch_backtest_atr import run_all_with_modes
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "backtest_report_merged_atr.md")
...
def plot_trade_process(records, trades, instrument, period_label, mode_label, output_path):
    ...
    ax1.set_title(f"{instrument} {period_label} {mode_label} - ATR动态阈值 交易过程图", fontsize=14, fontweight="bold")
    ax1.set_ylabel("价格")
...
def run_period(period_name, resample_fn, params):
    tmp_dir = os.path.join(DATA_DIR, f"{period_name}_tmp_atr")
...
def build_report(all_period_results):
    lines = []
    lines.append("# 布林带做空策略（ATR动态阈值） - 多周期回测融合报告")
    lines.append("")
    ...
    lines.append("| ATR 周期 | 14 | 14 | ATR 计算周期 |")
    lines.append("| ATR 倍数(N值) | 0.5 | 0.3 | 突破价 = 上轨 + N × ATR |")
    ...
    mode_labels = {"strict": "严谨（25%/ATR×0.5）", "relaxed": "宽松（20%/ATR×0.3）"}
    ...
    lines.append(f"- 严谨模式总交易: {total_trades_strict} 笔，总盈亏: {total_pnl_strict:>+,.1f} 元")
    lines.append(f"- 宽松模式总交易: {total_trades_relaxed} 笔，总盈亏: {total_pnl_relaxed:>+,.1f} 元")
    lines.append("")
...
def main():
    ...
    base_params = {
        ...
        "atr_period": 14,
        "bandwidth_threshold": 0.25, "atr_multiplier": 0.5,
        "fee_rate": 0.0001,
    }

    print("=" * 50)
    print("ATR动态阈值策略 - 多周期融合回测")
    print("=" * 50)
    ...
    for period_name, mode_key, mode_label, r in trade_entries:
        ...
        chart_filename = f"backtest_atr_{period_name.lower()}_{mode_key}_{inst}.png"
    ...
    for line in updated_lines:
        ...
        if line.startswith("### ") and " - " in line:
            ...
            if len(parts) >= 3:
                ...
                period_cn = parts[1]
                mode_part = parts[2]
                period_name = cn_to_en.get(period_cn, period_cn)
                mode_key = "strict" if "严谨" in mode_part else "relaxed"
                key = (period_name, mode_key, inst)
                if key in chart_refs:
                    final_lines.append("")
                    final_lines.append(f"![{inst} {period_cn} {mode_part} 过程图]({chart_refs[key]})")
                    final_lines.append("")
