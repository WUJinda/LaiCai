# ============================================================
# 布林带做多策略 (Bollinger Bands Long Strategy - Squeeze Breakout)
# 逻辑：前置条件满足(趋势向上+带宽宽) → 带宽收窄 → 价格再破上轨 → 做多
# 平仓：价格回落到中轨
# ============================================================
...
class Params(BaseParams):
    ...
    # 策略参数
    bandwidth_threshold: float = Field(default=0.21, title="带宽阈值")  # 21%
    squeeze_ratio: float = Field(default=0.7, title="收窄比例")  # 带宽收窄到峰值的70%视为收窄
...
class State(BaseState):
    ...
    bandwidth_peak: float = Field(default=0, title="带宽峰值")
    squeezed: bool = Field(default=False, title="带宽已收窄")


# ============================================================
# 策略主类
# ============================================================
class BollingerBandsLongStrategy(BaseStrategy):
    """布林带做多策略 - 带宽收窄后突破上轨做多"""
    ...
    def calc_signal(self, kline: KLineData) -> None:
        ...
        if not self.state_map.trend_confirmed:
            ...
            self.state_map.squeezed = False
            self.state_map.bandwidth_peak = 0
            return

        if self.check_preconditions():
            # 阶段1: 前置条件满足（趋势向上 + 带宽宽），记录峰值
            self.state_map.monitoring = True
            self.state_map.squeezed = False
            if self.state_map.bandwidth > self.state_map.bandwidth_peak:
                self.state_map.bandwidth_peak = self.state_map.bandwidth
        elif self.state_map.monitoring and self.state_map.bandwidth_peak > 0:
            # 阶段2: 前置条件不再满足（带宽开始收窄），检测收窄程度
            if self.state_map.bandwidth < self.state_map.bandwidth_peak * self.params_map.squeeze_ratio:
                self.state_map.squeezed = True
        else:
            # 无有效监听状态，重置
            self.state_map.monitoring = False
            self.state_map.squeezed = False
        ...
        if position.net_position > 0:
            if kline.close <= self.state_map.bb_middle:
                ...
                if self.trading:
                    ...
                    self.output(f"平多仓: 价格回落到中轨, 平仓价={kline.close}")

        # ========== 开仓逻辑 ==========
        # 带宽收窄后，价格再次突破上轨 → 做多
        if self.state_map.squeezed and position.net_position == 0:
            ...
            if kline.close > self.state_map.bb_upper:
                ...
                if self.trading:
                    ...
                    self.order_id.add(order_id)
                    self.output(f"开多仓: 带宽收窄后突破上轨, 开仓价={kline.close}, 上轨={self.state_map.bb_upper}")
